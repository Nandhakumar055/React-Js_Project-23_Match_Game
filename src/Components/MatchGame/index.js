...
class MatchGame extends Component {
  state = {
    ...
    randomBannerImageUrl: imagesList[0].imageUrl,
    randomBannerImageId: imagesList[0].id,
    isGameOver: false,
  }

  componentDidMount() {
    this.timerId = setInterval(this.renderSeconds, 1000)
  }
  ...
  getRandomBannerImage = () => {
    const numOfImageCount = imagesList.length
    const randomImageId = Math.floor(Math.random() * numOfImageCount)
    const randomImageUrl = imagesList[randomImageId].imageUrl
    this.setState({randomBannerImageUrl: randomImageUrl})

    const imageId = imagesList.filter(eachImage => {
      if (eachImage.imageUrl === randomImageUrl) {
        return this.setState({randomBannerImageId: eachImage.id})
      }
      return eachImage
    })
    return imageId
  ...
  winOrLose = id => {
    const {randomBannerImageId} = this.state
    if (randomBannerImageId === id) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.getRandomBannerImage()
    } else {
      this.setState(prevState => ({isGameOver: !prevState.isGameOver}))
      clearInterval(this.timerId)
    }
  ...
  render() {
    const {
      ...
      seconds,
      activeTabId,
      randomBannerImageUrl,
      isGameOver,
    } = this.state
    const filterCategoryImageList = this.filterCategoryImage()

    return (
      <div className="match-game-app-container">
        <nav className="nav-bar-container">
          <NavBar liveScore={score} runTimer={seconds} />
        </nav>
        <div className="body-container">
          <div className="main-section-bg-container">
            {isGameOver ? (
              <div className="game-over-section-container">
              ...
              <div className="main-section-container">
                <div className="image-banner-container">
                  <img
                    src={randomBannerImageUrl}
                    className="banner-image"
                    alt="match"
                  />
                </div>
                <div className="tab-item-section-container">
                  <div className="tab-item-container">
                    {tabsList.map(eachItem => (
                      <TabList
                        tabsListDetail={eachItem}
                        key={eachItem.tabId}
                ...
                <div className="thumbnail-images-section-container">
                  <div className="thumbnail-images-container">
                    {filterCategoryImageList.map(eachItem => (
                      <ImageItems
